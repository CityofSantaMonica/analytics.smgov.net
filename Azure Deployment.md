# Deploying to Azure with Travis CI

18F's original [analytics project](https://github.com/18F/analytics.usa.gov) was written with a Linux environment and [18F pages](https://github.com/18F/pages) in mind. Since we're not in a Linux environment, this project was simply ported to work with an Azure Web App.

This fork has both the Jekyll website and node app deployed to a single Azure Web App; everything remains on the same domain and in one place. Travis is being used to compile the Jekyll website and then deploy it to Azure.
...
- `AZURE_WA_PASSWORD` - The password of the user; also located in the portal
...
18F [specifies what environment variables](https://github.com/18F/analytics-reporter#setup) are needed in `.env` files. Instead of placing all of them in `.env` files and worry about sensitive information or repetition, store them Azure Application Settings.
...
Now that we have our already-built Jekyll site **and** our web jobs to poll Google Analytics, we have Kudu copy over the '\_site' folder (as specified by `DEPLOYMENT_SOURCE`).
